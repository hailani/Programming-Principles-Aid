...
const Main_Column = ({ column, tasks }) => { //Jayla - Column = information about the column (id + title) and Tasks = an array of task inside that specific column//
  return ( //Jayla - Line 8: creates the column box with the properties (rounded = rounded corners, bg = background color, w = width, h = height, flexDir = vertical layout)//
    <Flex rounded="10px" bg="columnBg" flex="1" w="100%" minH="600px" flexDir="column" border="1px solid" borderColor="cardBorder"> 
      <Flex //Jayla = lines 10-12 displays the column title
        align="center"
        h="60px"
        bg="columnHeaderBg"
        rounded="3px 3px 0 0" //Jayla - previous measurements: 3px 3px 0 0
        px="1.5rem"
        mb="1.5rem"
      >
...
export default Main_Column; //Jayla - Allows other files to import the Column component, which is used in index.js to display the columns on the page//
